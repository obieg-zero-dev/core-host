...
git clone https://github.com/obieg-zero-dev/{{PLUGIN_NAME}}    # to repo
...
cd ../{{PLUGIN_NAME}} && git add -A && git commit -m "..."
```
...
```bash
bash core-host/scripts/promote-to-dev.sh  {{PLUGIN_NAME}}             # LOCAL -> DEV
bash core-host/scripts/promote-to-prod.sh {{PLUGIN_NAME}} v1.2.3      # DEV   -> PROD + tag
```
